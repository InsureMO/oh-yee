...
const Grid = forwardRef<HTMLDivElement, GridProps>((baseprops, ref) => {
  ...
  const cls = clsx(
    prefixCls,
    [`${prefixCls}-columns-${resolvedCols}`],
    className,
  );
    ...
    <div className={cls} style={styles} id={id} ref={ref} role="grid">
      {items.map((_child, index: number) => {
        ...
        if (
          ...
          child.type !== null &&
          WRAPPERCOMPONENTS.includes((child.type as any).displayName)
        ) {
          innerChild = child.props
        ...
        const isWrappedGridItem =
          isWrappedByOverlay && innerChild?.type === Item;
        ...
        if (isGridItem) {
          return React.cloneElement(child, {
            ...
            spanStyle: {
              ...itemStyles,
              ...(child.props.spanStyle as Record<string, unknown>),
            },
        ...
        if (isWrappedGridItem && innerChild) {
          return React.cloneElement(child, {
            ...
            children: React.cloneElement(innerChild, {
              ...
              spanStyle: {
                ...itemStyles,
                ...((innerChild.props as GridItemProps).spanStyle as Record<
                  string,
                  unknown
                >),
              },

...
import type { TableSelectProps, InternalTableProps } from './interface';
import './style/index.less';

const handleState = (value: SelectionKeyType | SelectionKeyType[] | undefined) => {
...
const TableSelect = React.forwardRef((baseprops: TableSelectProps, ref) => {
    ...
    const selectorOptions = useMemo(() => {
        ...
        const translated = filtered.map((item) => ({
            ...
            label: typeof optionLabelProp === 'function' ? optionLabelProp(item) : item[optionLabelProp]
        }));
    ...
    const handleSearch = (value: string, e?: React.ChangeEvent<HTMLInputElement>) => {
        setSearchValue(value);
        searchOnInput && onSearch?.(value, e);
    };

    const clearSearchValue = () => {
        setSearchValue('');
        searchValue && onSearch?.('', undefined);
    };
    ...
    const onSelectChange = (
        ...
        const keys = selectionType === 'radio' && Array.isArray(selectedRowKeys) ? selectedRowKeys[0] : selectedRowKeys;
        const rows = selectionType === 'radio' && !Array.isArray(selectedRows) ? selectedRows : selectedRows as unknown as Array<unknown>;
    ...
    const onTagRemove = (obj: TagType) => {
        const keys = [...(mergedValue as SelectionKeyType[]).filter((k) => k !== obj?.value)];
        setMergedValue([...keys]);
        const renderKeys = keys.length === 0 ? (selectionType === 'checkbox' ? [] : '') : keys;
        onChange?.(renderKeys, undefined,);
    };

    const tableProps = omit({ ...rest, prefixCls, rowSelection: {
        ...props.rowSelection,
    ...
    const selectorProps = {
        ...
        mode: (selectionType === 'radio' ? undefined : 'multiple') as 'multiple' | 'tags' | undefined,

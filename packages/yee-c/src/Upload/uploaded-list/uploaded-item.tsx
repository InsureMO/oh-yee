...
import { CircleCheck, CircleX, Eye, Image, Paperclip, RotateCcw, Trash2 } from 'lucide-react';
        ...
        return (
            <>
                        ...
                        <>
                            {file.status === 'error' ? (
                                <div className={`${prefixCls}-picture-wall-item-error`}>
                                    {getShowCase()}
                                    <span className={`${prefixCls}-picture-wall-name`}>{file.name}</span>
                                </div>
                            ) : (
                ...
                <Dialog
                    ...
                    footer={false}
                >
                    <img src={previewImageSrc.current} alt="preview" style={{ maxWidth: '100%' }} />
                </Dialog>
            </>
        );
    }
                                ...
                                {file.status === 'error' ? (
                                    ...
                                    <>
                                        {fileType === 'image' && objectUrl ? (
                                            <img className={`${prefixCls}-picture-list-header-img`} src={objectUrl} alt={file.name} />
                                        ) : (
    ...
    const renderItem = () => {
        const node = (
            ...
            >
                ...
                {file.status !== 'uploading' && (
                    <Space className={`${prefixCls}-item-operate`} gap={8} onClick={(event) => event.stopPropagation()}>
                        {itemRender?.(file, [])}
        ...
        if (showTooltip) {
            return (
                <Tooltip  title={fileStatusTitle[file.status as FileStatus] || fileStatusTitle.ready}>
                    {node}
                </Tooltip>
            )
        }

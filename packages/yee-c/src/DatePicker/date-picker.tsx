...
const DatePicker = React.forwardRef<HTMLDivElement, DatePickerProps>((baseprops, ref) => {
  ...
  const { onBlur: onInputBlur, onChange: onInputChange, onKeydown: onInputKeydown } = useInput({
    setInputValue,
    setPickerValue,
    format,
  ...
  useEffect(() => {
    if (mergedValue) {
      ...
      setPickerValue(date);
      const inputValue = picker === 'week' ? `${date.year()}-${date.week()}` :
        picker === 'quarter' ? `Q${date.quarter()}` :

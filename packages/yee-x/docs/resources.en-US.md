---
title: Resources
order: 4
nav:
  title: Resources
  order: 8
---

# Resources

## Design Resources

### Design Guidelines

- [Design Principles](/design/principles) - Understand yee-x design philosophy
- [Visual Guidelines](/design/visual) - Color, typography, spacing specifications
- [Interaction Guidelines](/design/interaction) - Interaction patterns and animations

### Design Tools

- **Figma Component Library** - Coming soon
- **Sketch Component Library** - Coming soon
- **Icon Library** - Coming soon

## Development Resources

### Scaffolding and Templates

```bash
# Quick project creation (coming soon)
npx create-yee-app my-app
```

### Example Projects

- [Basic Chat App](https://github.com/InsureMO/oh-yee-examples/tree/main/basic-chat)
- [AI Assistant App](https://github.com/InsureMO/oh-yee-examples/tree/main/ai-assistant)
- [Document Q&A System](https://github.com/InsureMO/oh-yee-examples/tree/main/doc-qa)
- [Code Generator](https://github.com/InsureMO/oh-yee-examples/tree/main/code-generator)

### Code Snippets

#### VS Code Snippets

Install yee-x snippets extension (coming soon):

```bash
code --install-extension yee-x.snippets
```

## Learning Resources

### Tutorials

- [Quick Start](/guide) - Get started in 5 minutes
- [Advanced Guide](/advanced) - Deep dive into advanced features
- [Best Practices](/best-practices) - Production recommendations
- [Performance Optimization](/performance) - Performance tips

### Video Tutorials

- [yee-x Introduction](https://example.com/video/intro) - 30-minute quickstart
- [Building AI Chat App](https://example.com/video/chat-app) - Build from scratch
- [Component Deep Dive](https://example.com/video/components) - Core components explained
- [Real Project Development](https://example.com/video/project) - Complete project walkthrough

## Community Resources

### Official Channels

- [GitHub Repository](https://github.com/InsureMO/oh-yee)
- [GitHub Discussions](https://github.com/InsureMO/oh-yee/discussions)
- [Changelog](/changelog)

### Community Projects

- [awesome-yee-x](https://github.com/InsureMO/awesome-yee-x)
- [yee-x-plugins](https://github.com/InsureMO/oh-yee-plugins)
- [yee-x-themes](https://github.com/InsureMO/oh-yee-themes)

## Tools and Plugins

### Development Tools

- **Chrome DevTools Extension** - Coming soon
- **VS Code Extension** - Coming soon
- **ESLint Plugin** - Code linting
- **Prettier Config** - Code formatting

## Related Projects

### Ecosystem

- **yee-c** - Base component library
- **yee-tools** - Utility functions
- **yee-chat** - Chat application example
- **yee-mcp** - MCP protocol implementation

---

## Need Help?

If you encounter issues:

- Check [FAQ](/faq)
- Submit an [Issue](https://github.com/InsureMO/oh-yee/issues)
- Join [Discussions](https://github.com/InsureMO/oh-yee/discussions)
